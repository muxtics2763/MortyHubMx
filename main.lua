...
-- ══════════════════════════════════════════════════════════════
--  KEY SYSTEM  (v3 – hwid-key-manager API)
-- ══════════════════════════════════════════════════════════════
local API_BASE = "https://hwid-key-manager--mortyhub947.replit.app/api"
local KEY_FILE = "MortyHub_Key.txt"
local _keySecondsRemaining = 0

local function getHWID()
    local ok, id
    ok, id = pcall(function() return game:GetService("RbxAnalyticsService"):GetClientId() end)
    if ok and id then return tostring(id) end
    pcall(function() id = gethwid() end)
    if id then return tostring(id) end
    return "unknown-" .. tostring(game.JobId or "")
end
...
local function formatTime(secs)
    if secs >= 99999990 then return "Permanente" end
...
local function heartbeat(key, hwid)
    local url = API_BASE .. "/keys/" .. key .. "/heartbeat"
    local res, err = httpPostJson(url, jsonEncode({ hwid = hwid }))
    if not res then return false, err or "no response" end
    local data   = jsonDecode(res.Body or "") or {}
    local status = res.StatusCode or res.Status or 0
    if status == 200 and data.valid then
        return true, data.secondsRemaining
    elseif status == 403 then
        return false, data.message or "HWID mismatch or key expired"
    elseif status == 404 then
        return false, "Key not found"
    end
    return false, data.message or ("HTTP " .. tostring(status))
end

local function kickWithMessage(msg)
    local LP  = game:GetService("Players").LocalPlayer
    local pg  = LP:WaitForChild("PlayerGui")
    local sg  = Instance.new("ScreenGui")
    sg.Name = "MortyHubExpired"; sg.ResetOnSpawn = false
    ...
    card.Size = UDim2.fromOffset(340, 160)
    ...
    icon.Font = Enum.Font.GothamBold; icon.TextSize = 28
    icon.TextColor3 = Color3.fromRGB(255,60,60)
    icon.Text = "KEY EXPIRED"

    local sub = Instance.new("TextLabel", card)
    sub.BackgroundTransparency = 1
    sub.Position = UDim2.new(0,16,0,64)
    sub.Size = UDim2.new(1,-32,0,20)
    sub.Font = Enum.Font.Gotham; sub.TextSize = 13
    sub.TextColor3 = Color3.fromRGB(200,200,200)
    sub.Text = msg or "Tu key ha expirado."
    sub.TextWrapped = true

    local sub2 = Instance.new("TextLabel", card)
    sub2.BackgroundTransparency = 1
    sub2.Position = UDim2.new(0,16,0,90)
    sub2.Size = UDim2.new(1,-32,0,18)
    sub2.Font = Enum.Font.Gotham; sub2.TextSize = 11
    sub2.TextColor3 = Color3.fromRGB(150,150,150)
    sub2.Text = "Cerrando sesión en 5 segundos..."

    task.delay(5, function()
        pcall(function() game:GetService("Players").LocalPlayer:Kick("MortyHub: Key Expired") end)
    end)
end

local function startHeartbeatLoop(key, hwid)
    task.spawn(function()
        while true do
            task.wait(15)
            local ok, info = heartbeat(key, hwid)
            if not ok then
                _keySecondsRemaining = 0
                kickWithMessage("Tu key ha expirado o fue desactivada.")
                break
            end
            _keySecondsRemaining = info
        end
    end)
end

local function promptKey(hwid, errorMsg)
    local LP  = game:GetService("Players").LocalPlayer
    local pg  = LP:WaitForChild("PlayerGui")
    local old = pg:FindFirstChild("MortyHubKeyGui")
    if old then old:Destroy() end

    local sg = Instance.new("ScreenGui")
    sg.Name = "MortyHubKeyGui"; sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true; sg.AutoLocalize = false; sg.Parent = pg
    ...
    local card = Instance.new("Frame", bg)
    card.Size = UDim2.fromOffset(320,230); card.Position = UDim2.new(0.5,-160,0.5,-115)
    card.BackgroundColor3 = Color3.fromRGB(20,24,32); card.BorderSizePixel = 0
    Instance.new("UICorner", card).CornerRadius = UDim.new(0,12)
    local stroke = Instance.new("UIStroke", card)
    stroke.Color = Color3.fromRGB(58,134,255); stroke.Thickness = 1.5

    local title = Instance.new("TextLabel", card)
    title.BackgroundTransparency = 1; title.Position = UDim2.new(0,16,0,12)
    title.Size = UDim2.new(1,-32,0,24); title.Font = Enum.Font.GothamBold
    title.TextSize = 18; title.TextColor3 = Color3.fromRGB(255,255,255)
    title.TextXAlignment = Enum.TextXAlignment.Left; title.Text = "MortyHub | Key"

    local sub = Instance.new("TextLabel", card)
    sub.BackgroundTransparency = 1; sub.Position = UDim2.new(0,16,0,38)
    sub.Size = UDim2.new(1,-32,0,16); sub.Font = Enum.Font.Gotham; sub.TextSize = 11
    sub.TextColor3 = Color3.fromRGB(150,160,180)
    sub.TextXAlignment = Enum.TextXAlignment.Left
    sub.Text = "Paste your key to activate the hub"

    local box = Instance.new("TextBox", card)
    box.Position = UDim2.new(0,16,0,64); box.Size = UDim2.new(1,-32,0,36)
    box.BackgroundColor3 = Color3.fromRGB(30,36,48); box.BorderSizePixel = 0
    ...
    errLbl.BackgroundTransparency = 1; errLbl.Position = UDim2.new(0,16,0,110)
    errLbl.Size = UDim2.new(1,-32,0,16); errLbl.Font = Enum.Font.Gotham; errLbl.TextSize = 11
    errLbl.TextColor3 = Color3.fromRGB(255,90,100)
    errLbl.TextXAlignment = Enum.TextXAlignment.Left; errLbl.Text = errorMsg or ""

    local btn = Instance.new("TextButton", card)
    btn.Position = UDim2.new(0,16,1,-56); btn.Size = UDim2.new(1,-32,0,40)
    btn.BackgroundColor3 = Color3.fromRGB(58,134,255); btn.BorderSizePixel = 0
    btn.AutoButtonColor = true; btn.Font = Enum.Font.GothamBold; btn.TextSize = 14
    btn.TextColor3 = Color3.fromRGB(255,255,255); btn.Text = "Activate"
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0,8)

    local result = nil
    btn.MouseButton1Click:Connect(function()
        local k = box.Text:gsub("%s+","")
        if k == "" then errLbl.Text = "Enter a key"; return end
        btn.Text = "Validating…"; btn.AutoButtonColor = false
        local ok, info = heartbeat(k, hwid)
        if ok then
            saveKey(k)
            _keySecondsRemaining = info
            startHeartbeatLoop(k, hwid)
            sg:Destroy()
            result = true
        else
            btn.Text = "Activate"; btn.AutoButtonColor = true
            errLbl.Text = "Error: " .. tostring(info)
        end
    end)

    repeat task.wait() until result ~= nil or not sg.Parent
    return result == true
end

local function gateKey()
    local hwid  = getHWID()
    local saved = readSavedKey()
    if saved and saved ~= "" then
        local ok, info = heartbeat(saved, hwid)
        if ok then
            _keySecondsRemaining = info
            startHeartbeatLoop(saved, hwid)
            return true
        end
        deleteSavedKey()
        return promptKey(hwid, "Saved key is no longer valid")
    end
    return promptKey(hwid, nil)
end

if not gateKey() then return end
